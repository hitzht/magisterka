...
def perform_tests(test_instances, iterations, permutations_count, distance_factor):
    ...
    for instance in test_instances:
        ...
        average_total_time = sum([x[3] for x in results]) / tests_per_iteration

        print(instance_name, best_found, best_diff, round(average_value, 2), round(average_diff, 2),
              round(average_time, 2), round(average_total_time, 2))
...
if __name__ == '__main__':
    ...
    perform_tests(test_instances, 500, 100, 0.5)
    perform_tests(test_instances, 500, 300, 0.5)
    perform_tests(test_instances, 500, 500, 0.5)

    perform_tests(test_instances, 3000, 100, 0.5)
    perform_tests(test_instances, 3000, 300, 0.5)
    perform_tests(test_instances, 3000, 500, 0.5)

    perform_tests(test_instances, 500, 100, 0.2)
    perform_tests(test_instances, 500, 100, 0.6)
    perform_tests(test_instances, 500, 100, 1)

    perform_tests(test_instances, 2000, 200, 0.2)
    perform_tests(test_instances, 2000, 200, 0.6)
    perform_tests(test_instances, 2000, 200, 1)
